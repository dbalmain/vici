...
The comparison below is against the sibling `beetle` experiment's TypeScript
engine, which implements the same contract; its WASM column is in that project's
own report.
...
`vici-oracle`. There is no second Rust definition of the format to drift from.
